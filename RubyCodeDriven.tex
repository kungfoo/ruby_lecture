...
\section{Named Arguments simulieren} % (fold)
\label{sec:named_arguments_simulieren}
Ruby hat keine benannten Argumente, wie zum Beispiel Lisp. Allerdings gibt es einen (in der Zwischenzeit fast idiomatischen) Weg, wie benannte Argumente für Methoden implementiert werden können. Die Implementation verwendet Hashes und sieht wie folgt aus:

\lstinputlisting{code/named_args.rb}

Der Witz dieser Methode ist es, dass es ganz einfach ist, die verschiedenen Argumente mit Standardwerten zu versehen und dass diese überschrieben werden, wenn der Benutzer den richtigen Namen angibt. Der erste Parameter muss dabei aber angegeben werden.
...
\end{lstlisting}
Wir verwenden hierbei ein Array von Strings um den fertigen String zusammenzusetzen. Damit verhindern wir das unnötige Allozieren von vielen temporären Stringobjekten, wie sie anfallen würden, wenn wir den Operator \texttt{+=} verwenden würden um den String zusammenzusetzen.
