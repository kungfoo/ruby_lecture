...
\label{sec:methoden}
Methoden in Ruby werden durch einen '.' an eine Variable oder Konstante angehängt. Es gilt die Konvetion, dass Methoden die einen \texttt{boolean} zurückliefern am Ende ihres Namens ein '?' tragen. Methoden, die eine spezielle Version einer anderen Methode sind, werden mit einem '!' markiert. So geben zum Beispiel in der Basisbibliothek viele Methoden eine \emph{veränderte Kopie} des Objektes zurück. Dies hat einige Vorteile\footnote{Siehe dazu zum Beispiel das Kapitel Promote Immutability in \emph{Effective Java von Joshua Bloch}}, aber manchmal möchte man aber keine Kopie erhalten und die Methoden, die jeweils keine Kopie zurückgeben sind mit '!' markiert:
