...
Duck-Typing bedeutet dass erst zur Laufzeit überprüft wird, ob ein Objekt über bestimmte Merkmale — wie zum Beispiel Methodennamen und Attribute — verfügt, was zu einer erhöhten Flexibiltät führt. Es wird aber auch die Möglichkeit reduziert, Typen zur Compilezeit statisch zu prüfen und solche Fehler im Programm zu finden. Richtig angewendet kann Duck-Typing ihr Programm aber um einiges einfacher machen.
...
\begin{itemize}
	...
	\item Die Codezeile 17 iteriert über alle Vögel in \texttt{birds} und falls \texttt{bird.respond\_to? :quack} (also wenn das Objekt die Methode \texttt{respond\_to?} hat) dann wird diese Methode aufgerufen. Andernfalls ist der Vogel keine Ente.
\end{itemize}
...
Code wie der obige lässt sich mit \texttt{instanceof()} und einem Interface \texttt{CanQuack} auch noch relativ kurz und prägnant in Java oder ähnlichen Sprachen schreiben. Er wird aber schnell einiges komplizierter. Mehr dazu aber später.
% subsection duck_typing (end)


\subsection*{(Fast) komplett platformunabhängig}\label{sub:platforms}
Es gibt viele Platformen, auf denen Ruby läuft. Man hat zum Beispiel die Auswahl zwischen MRI\footnote{\url{http://www.ruby-lang.org}} und JRuby\footnote{\url{http://www.jruby.org/}}. Mit JRuby kann Ruby-Code auf der JVM (und damit auf allen Systemen wo eine JVM existiert) ausgeführt werden. MRI selbst ist auch auf praktisch allen Platformen verfügbar. Sie können die Beispiele in dieser Vorlesung sowohl mit MRI Ruby 1.8, 1.9 oder JRuby ausführen. Der Einfachheit halber verwenden Sie am besten die am weitesten verbreitete Version 1.8.7 von MRI.

Probleme machen nur Ruby-Libraries, die nativen Code verwenden, der zum Beispiel in C geschrieben ist. Solange ein Programm aber nur in Ruby geschrieben ist, gibt es mit der Portierbarkeit generell keine grossen Probleme.
...
Die Syntax von Ruby ist kurz, prägnant und sprechend. Ruby wurde bewusst als »sprechbare« Sprache designt. Die Idee dahinter ist, dass wenn eine Programmiersprache der natürlichen Sprache ähnelt, sie auch einfacher zu lesen und verstehen ist.
...
Es gibt in Ruby noch eine weitere Form Strings zu schreiben und das sind Multiline-Strings. Diese sind zum Beispiel dann nützlich, wenn ein String über mehrere Zeilen gehen soll und auch tatsächlich Newslines im String enthalten sein sollen.

Hier ein Beispiel:

\lstinputlisting{code/multiline.txt}

Dieser String wird genau so, wie er hier im Source Code zwischen dem frei wählbaren Delimiter 'TEXT' steht auch in ihrem Programm verfügbar sein, inklusive der gesamten Formatierung.
...
Als Vergleich: Ein String-Literal (auch in Java) erzeugt \emph{immer} ein neues String-Objekt. Normalerweise verwendet man Symbols dort, wo ein Name gebraucht wird, der aber nie geändert werden soll, wie zum Beispiel Namen von Methoden oder Hash-Elementen. (Siehe auch das Beispiel zu Duck-Typing)
...
Weiter können auch mehrere Variablen elegant und kurz auf Standardwerte gesetzt werden. Die \emph{kann} die Lesbarkeit erhöhen, kann aber auch für unnötige Verwirrung sorgen, den Einsatz einer parallelen Zuweisung in diesem Sinne abwägen.
...
\label{sec:methoden}
Methoden in Ruby werden durch einen '.' an eine Variable oder Konstante angehängt. Es gilt die Konvetion, dass Methoden die einen \texttt{boolean} zurückliefern am Ende ihres Namens ein '?' tragen.

Methoden, die eine spezielle Version einer anderen Methode sind, werden mit einem '!' markiert. So geben zum Beispiel in der Basisbibliothek viele Methoden eine \emph{veränderte Kopie} des Objektes zurück. Dies hat einige Vorteile\footnote{Siehe dazu zum Beispiel das Kapitel Promote Immutability in \emph{Effective Java von Joshua Bloch}}, aber manchmal möchte man aber keine Kopie erhalten und die Methoden, die jeweils keine Kopie zurückgeben sind mit '!' markiert:

Hier ein Beispiel dazu mit der Methode \texttt{String\#gsub}:

\lstinputlisting{code/gsub.rb}
...
Am besten lassen sich die verschiedenen Aspekte von Klassen in Ruby direkt an einem ganzen Beispiel aufzeigen:
...
\begin{enumerate}
	\item \texttt{attr\_accessor} generiert Getter und Setter für Properties. (Und nimmt als Werte offenbar \emph{Symbols} gerne entgegen...)
...
Interessant wird das Ganze, wenn Code aus dem Modul Methoden aus der Klasse verwenden kann um neue Methoden zu definieren.
...
\begin{itemize}
	\item \texttt{find}: Findet das erste Element in diesem \texttt{Enumerable}, für das ein angegebenes Prädikat \texttt{true} ist.
	\item \texttt{find\_all}: Findet aller Elemente für die ein Prädikat \texttt{true} ist.
	\item \texttt{include?}: Gibt es ein Element, für welches das Prädikat \texttt{true} ist?
	\item \texttt{collect}
	\item \texttt{sort}
\end{itemize}

Mehr zu diesen Methoden gleich im nächsten Abschnitt, wenn wir die Art und Weise wie solche Prädikate einfach und angegeben werden können kennen gelernt haben.
...
Nun als weiteres Beispiel die versprochene Verwendung der Methoden \texttt{Enumerable\#find} und \texttt{collect}:

\lstinputlisting{code/blocks3.rb}
...
Auffallend ist hierbei, dass die Argumente von hinten nach vorne ihre default-Werte erhalten. Wird also nur 4 als Wert angegeben, so gibt die Methode das Resultat von 4*3 = 12 aus.


Eine variable Anzahl Argumente kann in Ruby erreicht werden indem der Name des Arguments mit einem \emph{Splat} versehen wird. Logischerweise kann nur das letzte Argument einen Splat tragen, denn wie sollte Ruby sonst wissen, wo die variable Anzahl Argumente anfängt und wo sie aufhört.
...
Achten Sie darauf, dass zum Beispiel das Jahr 2009 kein Schaltjahr ist, also haben wir im Bereich zwischen dem 20.02.2009 und 12.03.2009 keinen Schalttag. Sie können diese Funktionalität ganz einfach auch in einer ihrer Klassen bereitstellen. Sie brauchen wie gesagt, nicht mehr zu tun als die Methode \texttt{next} zu definieren.
...
\label{sub:case}
\begin{quotation}
	\swashed{\Huge“\normalsize The Ruby \texttt{case} expression is a powerful beast: a multiway \texttt{if}\\* on steroids.\Huge”\normalsize — Programming Ruby, The Pragmatic Programmers Guide }
\end{quotation}
...
Sie lassen sich auch als Modifier für einzelne Statements verwenden, was natürlich die Lesbarkeit erhöhen kann, wie man das zum Beispiel schön ist Zeile 6 des nachfolgenden Beispiels sehen kann.
...
Reguläre Ausdrücke werden entweder zwischen /\emph{Ausdruck}/ oder mit dem Shorthandle \%r\{\emph{Ausdruck}\} geschrieben.
...
\section{Named Arguments} % (fold)
\label{sec:named_arguments_simulieren}
Ruby hat keine benannten Argumente, wie zum Beispiel Lisp. Allerdings gibt es einen (in der Zwischenzeit fast idiomatischen) Weg, wie benannte, optionale Argumente für Methoden implementiert werden können. Die Implementation verwendet Hashes und sieht dabei ungefähr wie folgt aus:

\lstinputlisting{code/named_args.rb}

Der Witz dieser Methode ist, dass es ganz einfach ist, die verschiedenen Argumente mit Standardwerten zu versehen und dass diese überschrieben werden, wenn der Benutzer den richtigen Namen angibt.

Der erste Parameter muss dabei aber angegeben werden (er hat ja keinen Defaultwert).
Viele Frameworks die in Ruby geschrieben sind, verwenden diese Art der Parameterübergabe. 

Auch dies ist ein schönes Beispiel dafür, dass das Erweitern von bereits existierenden Klassen durchaus nützlich sein kann, wenn man es richtig verwendet. Die Klasse \texttt{Hash} wird hier um die Methode \texttt{default!} erweitert wird, die uns das Setzen der Optionen ganz einfach macht.
% section named_arguments_simulieren (end)
...
\end{lstlisting}
Wir verwenden hierbei ein Array von Strings um den fertigen String zusammenzusetzen. Damit verhindern wir das unnötige Allozieren von vielen temporären Stringobjekten, wie sie anfallen würden, wenn wir den Operator \texttt{+=} verwenden würden um den fertigen Inhalt der Datei zusammenzusetzen.
...
\begin{itemize}
	...
	\item \texttt{kind\_of?}: gleich wie \texttt{instance\_of?}, gibt aber \texttt{true} zurück, auch wenn das Objekt eine Instanz einer Subklasse ist oder wenn es sich um ein Mixin handelt, wenn die Klasse das Modul als Mixin verwendet.
...
\label{sec:threads}
Threads starten ist dank Closures in Ruby denkbar einfach:
